...
def load_for_org(org_hash):
    ...
    # assume project present except check for default-catalog
    if config["PROJECT_NAME"] == "default-catalog":
        create_project()  
    load_all_resources()
    print(f"======= Completed loading templates for org {org_hash} =======")

def main(csv_file='api.csv'):
    """
    Reads partner information from CSV file 
    """
    ...
    with open(csv_file, mode='r', encoding='utf-8') as file:
        ...
        for row in reader:
            ...
            hostenv = row.get('hostenv')
            opsenv = row.get('opsenv')
            apikey = row.get('apikey')
            prj = row.get('project')
            print(f"Got input of loading templates for: {load}, partner: {partner}, hostenv: {hostenv}, opsenv: {opsenv}, in project: {prj}")
            if load == 'y':
                print(f"================================================================")
                print(f"======= Loading for {partner} =========================================================")
                HOST_ENV = hostenv
                OPS_ENV = opsenv
                PROJECT_NAME = prj
                BASE_URL = f"https://{HOST_ENV}"
                ...
                config["HOST_ENV"]= hostenv
                config["API_KEY"]=apikey
                config["PROJECT_NAME"]= prj
                #print(config)
                load_templates()
                print(f"======= Finished Loading for {partner} =========================================================")
                print(f"================================================================")
            else:
                print(f"Skipping Loading for {partner} ")

if __name__ == "__main__":
    read_values(partner_api_key=None, values_file="values.yaml")
    main(csv_file='api.csv')
